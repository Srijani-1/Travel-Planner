...
import { Globe, Shield, Cloud, MapPin, Badge } from "lucide-react";
import { motion } from "motion/react";
import { useEffect, useState } from "react";
import { useDashboardStats } from "../../hooks/useDashboard";
import { sampleRecommendations, type SampleRec } from "../../data/sampleRecommendations";
...
// ─── Build plan-trip URL with all pre-fill params ─────────────────────────────
function buildPlanUrl(trip: SampleRec): string {
  const params = new URLSearchParams({
    destination: trip.destination,
    duration: String(trip.durationDays),
    preferences: trip.preferences.join(","),
    budget: String(trip.budgetValue),
    stayType: trip.stayType,
    dietary: trip.dietary,
    autofill: "true",
  });
  return `/dashboard/plan-trip?${params.toString()}`;
}
  ...
  return (
    ...
    >
      <Card className="overflow-hidden border border-white/40 dark:border-white/10 bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl hover:shadow-2xl transition-all duration-500 cursor-pointer group rounded-3xl h-full flex flex-col ring-1 ring-black/5">
        {/* ── Photo ── */}
        <div className="relative h-52 overflow-hidden mx-3 mt-3 rounded-2xl bg-slate-100 dark:bg-slate-800">
          {/* shimmer while loading */}
          {!imgLoaded && !imgError && (
            <div className="absolute inset-0 bg-gradient-to-r from-slate-200 via-slate-100 to-slate-200 dark:from-slate-700 dark:via-slate-600 dark:to-slate-700 animate-pulse z-10" />
          )}
          <img
          ...
          />
          <div className="absolute inset-0 bg-blue-900/15 group-hover:bg-transparent transition-colors duration-500" />
        ...
        <CardContent className="mt-auto pb-5">
          ...
          <div className="flex flex-wrap gap-1.5 mb-4">
            {trip.preferences.map(p => (
              <span
                key={p}
                className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-slate-300 uppercase tracking-wide"
              >
                {p}
              </span>
            ))}
          ...
          <Button
            variant="outline"
            className="w-full rounded-xl hover:bg-gradient-to-r hover:from-pink-500 hover:to-rose-600 hover:text-white hover:border-transparent transition-all duration-300 border-2 font-bold"
            onClick={() => navigate(buildPlanUrl(trip))}
          >
            ✨ Plan This Trip
          </Button>
        </CardContent>
  ...
  return (
    <div className="p-6 lg:p-10 max-w-7xl mx-auto space-y-12">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard icon={MapPin} label="Planned" value={stats?.trips_planned || 0} color="from-blue-600 to-indigo-600" delay={0.1} />
        <StatCard icon={Badge} label="Visited" value={stats?.trips_completed || 0} color="from-emerald-600 to-teal-600" delay={0.2} />
        <StatCard icon={Globe} label="Saved" value={stats?.saved_places || 0} color="from-amber-500 to-orange-600" delay={0.3} />
        <StatCard icon={Globe} label="Regions" value={stats?.countries_visited || 0} color="from-violet-600 to-purple-700" delay={0.4} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <Button variant="link" onClick={() => navigate("/dashboard/my-trips")} className="font-bold text-pink-600 text-xs uppercase tracking-widest">
              View All
            </Button>
          </div>
          ...
          <div className="flex flex-col gap-4">
            {statsLoading ? (
              ...
              stats.upcoming_trips.map((trip: any, i: number) => (
                <Card
                  key={i}
                  className="bg-white/90 dark:bg-zinc-900/90 backdrop-blur-md border-slate-200 dark:border-white/10 shadow hover:shadow-xl transition-all duration-300 cursor-pointer rounded-3xl group"
              ...
              <div className="text-center py-12 bg-slate-50 dark:bg-white/5 border-2 border-dashed border-slate-200 dark:border-white/10 rounded-3xl">
                <p className="text-slate-500 dark:text-slate-400 font-bold">
                  No upcoming trips.{" "}
                  <button onClick={() => navigate("/dashboard/plan-trip")} className="text-pink-600 underline">
                    Plan one!
                  </button>
                </p>
              </div>
            ...
            <div className="rounded-3xl p-6 bg-gradient-to-r from-pink-500/10 to-rose-500/10 border border-pink-200 dark:border-pink-800/50 flex items-center justify-between gap-6 backdrop-blur-sm">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-pink-700/70 dark:text-pink-400/70 font-bold uppercase tracking-widest mt-1">
                    Verified rides, stays & SOS contacts linked
                  </p>
                </div>
              </div>
        ...
        >
          ...
          {firstUpcoming ? (
            <WeatherWidget city={firstUpcoming.name} lat={firstUpcoming.lat || 19.076} lon={firstUpcoming.lon || 72.8777} />
          ) : (
          ...
          <Card className="bg-slate-900 text-white rounded-3xl p-6 shadow-2xl border-0 overflow-hidden relative group">
            ...
            <p className="font-bold text-sm leading-relaxed">
              Always check the local 'Women Only' transportation options in{" "}
              {firstUpcoming?.name.split(",")[0] || "your destination"} before arriving.
            </p>
          </Card>
        </motion.div>
      </div>
      ...
      >
        <div className="flex flex-col gap-1">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm font-bold uppercase tracking-widest">
            Handpicked destinations · click to auto-fill your trip plan
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {sampleRecommendations.map((trip, index) => (
            <RecommendationCard key={trip.destination} trip={trip} index={index} navigate={navigate} />
          ))}
        </div>
      </motion.div>
    </div>
