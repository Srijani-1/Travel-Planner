...
import {
    ...
    Lightbulb, Star, Shield, ChevronRight, X, Send, Loader2,
    CheckCircle2, Globe, Lock, Filter,
} from "lucide-react";
...
type Group = {
    ...
    category: string; member_count: number; is_public: boolean; is_member: boolean;
};
type Post = {
...
export function Community() {
    ...
    const [newPost, setNewPost] = useState({
        title: "", body: "", post_type: "general", location: "", group_id: "" as string | number,
    });
    const [savingPost, setSavingPost] = useState(false);

    // new review form
    const [newReview, setNewReview] = useState({
        place_name: "", place_type: "area", location: "",
        rating: 0, safety_rating: 0, body: "",
        tags: [] as string[],
    });
    ...
    const toggleMembership = async (group: Group) => {
        try {
            if (group.is_member) {
                await fetch(`${BASE}/community/groups/${group.id}/leave`, { method: "DELETE", headers: headers() });
    ...
    const submitReview = async () => {
        if (!newReview.place_name || !newReview.body || newReview.rating === 0) {
            toast.error("Fill in place name, a review, and at least one star");
            return;
        }
        setSavingReview(true);
        try {
            await post("/community/reviews", {
                ...newReview,
                location: newReview.location || null,
                safety_rating: newReview.safety_rating || null,
            });
                                    ...
                                    {filteredPosts.map((post, i) => {
                                        ...
                                        return (
                                            <motion.div key={post.id}
                                                ...
                                                <Card className={`hover:shadow-lg transition-all duration-200 rounded-3xl overflow-hidden
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                            ...
                            <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-slate-100 dark:border-white/10">
                                ...
                                <button onClick={() => setShowNewPost(false)} className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-white/5">
                                    <X className="h-4 w-4" />
                                </button>
                            </div>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                            ...
                            <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-slate-100 dark:border-white/10 sticky top-0 bg-white dark:bg-zinc-900 z-10">
                                ...
                                <button onClick={() => setShowNewReview(false)} className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-white/5">
                                    <X className="h-4 w-4" />
                                </button>
                            </div>
                            <div className="p-6 space-y-4">
                                <Input placeholder="Place name *" value={newReview.place_name}
                                    onChange={e => setNewReview(r => ({ ...r, place_name: e.target.value }))}
                                    className="rounded-xl" />
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        <Input placeholder="e.g. well-lit, women-only" value={tagInput}
                                            ...
                                            onKeyDown={e => {
                                                if (e.key === "Enter" && tagInput.trim()) {
                                                    setNewReview(r => ({ ...r, tags: [...r.tags, tagInput.trim().toLowerCase()] }));
                                                    setTagInput("");
                                                }
                                            }}
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {newReview.tags.map(tag => (
                                                <span key={tag} className="flex items-center gap-1 px-3 py-1 rounded-full bg-slate-100 dark:bg-white/5 text-xs font-bold">
                                                    #{tag}
                                                    <button onClick={() => setNewReview(r => ({ ...r, tags: r.tags.filter(t => t !== tag) }))}
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                            ...
                            <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-slate-100 dark:border-white/10">
                                ...
                                <button onClick={() => setShowNewGroup(false)} className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-white/5">
                                    <X className="h-4 w-4" />
                                </button>
                            </div>
                            <div className="p-6 space-y-4">
